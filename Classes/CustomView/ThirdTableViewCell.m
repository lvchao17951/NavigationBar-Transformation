...
- (void)createMyCell
{
    ...
    self.detialLbebl = [[UILabel alloc] init];
    [self.contentView addSubview:self.detialLbebl];
}
...
- (void)layoutSubviews
{
    ...
    [self.detialLbebl mas_makeConstraints:^(MASConstraintMaker *make)
    {
        make.size.mas_equalTo(CGSizeMake(122, 24));
        make.top.mas_equalTo(20);
        make.right.mas_equalTo(0);
    }];
    self.detialLbebl.font = [UIFont systemFontOfSize:14];
    self.detialLbebl.textColor = [UIColor colorWithWhite:0.6f alpha:1.0f];
    self.detialLbebl.textAlignment = 0;
}
